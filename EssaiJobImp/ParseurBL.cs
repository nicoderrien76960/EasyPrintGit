    ...
    class ParseurBL
    {
        ...
        public void miseEnForm(string typeDoc)
        {
            ...
            while (incCopie < nbCopie)
            {
                ...
                if (donneEntete["Bon_typvte"] == "EMPORTEE" || donneEntete["Camion_code"] == "ENV")
                {
                ...
                else
                {
                    if (donneEntete["Tiers_adl1"] == "")
                    {
                        ...
                        pAdl.Add(new Phrase(donneEntete["Tiers_adfcp"] + "   " + donneEntete["Tiers_adl6"] , FontFactory.GetFont(FontFactory.HELVETICA, 9, Font.BOLD)));
                ...
                tabCell.AddCell(new Phrase("N° BL", FontFactory.GetFont(FontFactory.HELVETICA, 11, Font.BOLD)));
                tabCell.AddCell(new Phrase(donneEntete["Client_code"].Trim()+"\n", FontFactory.GetFont(FontFactory.HELVETICA, 9)));
                tabCell.AddCell(new Phrase(donneEntete["Document_date"].Trim()+"\n", FontFactory.GetFont(FontFactory.HELVETICA, 9)));
                tabCell.AddCell(new Phrase(donneEntete["Document_numero"].Trim()+"     ", FontFactory.GetFont(FontFactory.HELVETICA, 9)));
                ...
                table.TotalWidth = 545;                                                                                         //Chaque colonne crée ci dessus doit être rempli
                ...
                Chunk chrono = new Chunk("Les marchandises et matériels objets de ce BL restent notre propriété jusqu'au paiement intégral de leur prix. (Loi 81-355 du 12 Mai 1980) ", FontFactory.GetFont(FontFactory.HELVETICA, 9));
                ...
                float[] largeursCartouche = { 45, 35, 25 };
                PdfPTable cartouche = new PdfPTable(largeursCartouche);
                cartouche.TotalWidth = 555;
                cartouche.LockedWidth = true;
                cartouche.AddCell(new Phrase("Cachet de l'entreprise : \n\n\n\n\n"));
                cartouche.AddCell(new Phrase("Nom :\n\nSignature : \n\n"));
                cartouche.AddCell(new Phrase("Date :         /         /20" + "\n\nHeure :          h    "));
                PdfPCell finDoc = new PdfPCell(new Phrase("Toute réclamation doit être faite dans les 48h après la livraison. La date du BL faisant foi.", FontFactory.GetFont(FontFactory.HELVETICA, 12, Font.BOLD)));
                finDoc.Colspan = 3;
                cartouche.AddCell(finDoc);
                ...
                { 
                    ...
                    { LogHelper.WriteToFile(e.Message, "ParseurBL" + donneEntete["Document_numero"].Trim()); }
